...
public class PageData {
    ...
    public void fetchData() throws TException {
        ...
        try {
            ...
            TReplicationService.Client client =
                    new TReplicationService.Client(protocol);

            retiredJobs = new ArrayList<>();
            long marker = -1;
            while(true) {
                List<TReplicationJob> jobBatch =
                        client.getRetiredJobs(marker, 100);
                if (jobBatch.size() == 0) {
                    break;
                }
            ...
            while(true) {
                List<TReplicationJob> jobBatch =
                        client.getActiveJobs(marker, 100);
                if (jobBatch.size() == 0) {
                    break;
                }
