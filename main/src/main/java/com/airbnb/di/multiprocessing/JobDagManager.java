...
/**
 * Given jobs (that need to get specific locks before running), this class helps
 * create and manage a DAG of jobs. For example, job 1 needs lock a, job 2 needs
 * lock b and job 3 needs lock a and lock b. The DAG would look like:
 *
 *  (job 1, job 2) -> job 3
 *
 *  job 1 and job 2 would run in parallel, and once those 2 are done, job 3 can
 *  run.
 */
public class JobDagManager {
    ...
    private void removeLockToJobsNeedingLock(String lock, Job job,
                                             boolean shouldBeAtHead) {
        ArrayList<Job> jobs = lockToJobsNeedingLock.get(lock);
        if (shouldBeAtHead && jobs.get(0) != job) {
            throw new RuntimeException("Tried to remove " + job + " but it " +
                    "wasn't at the head of the list for lock " + lock + "! List is: "
                    + jobs);
        }
        boolean removed = jobs.remove(job);
        if (!removed) {
            throw new RuntimeException("Didn't remove job " + job +
                    " from list " + jobs);
        }
    }
    ...
    private void grantExclusiveLock(String lock, Job job) {
        if (exclusiveLocksHeld.containsKey(lock)) {
            throw new RuntimeException("Tried to give exclusive lock to " +
                job + " when it was held by " + exclusiveLocksHeld.get(lock));
        }
    ...
    private void grantSharedLock(String lock, Job job) {
        if (exclusiveLocksHeld.containsKey(lock)) {
            throw new RuntimeException("Tried to give shared lock " + lock +
                    " to " + job + " when an exclusive lock was held by " +
    ...
    synchronized public boolean addJob(Job jobToAdd) {
        ...
        for (String lockToGet : lockSet.getSharedLocks()) {
            ...
            for(int i = jobs.size() - 1; i >=0; i--) {
                Job otherJob = jobs.get(i);
                Lock.Type requiredLockType =
                        otherJob.getRequiredLocks().getType(lockToGet);
    ...
    private boolean hasSharedLock(String sharedLock, Job job) {
        return sharedLocksHeld.containsKey(sharedLock) &&
                sharedLocksHeld.get(sharedLock).contains(job);
    }

    private void removeExclusiveLock(String exclusiveLock, Job job) {
        if (!hasExclusiveLock(exclusiveLock, job)) {
            throw new RuntimeException("Job " + job + " was supposed to " +
                    "have exclusive lock " + exclusiveLock +
                    " but it didn't!");
        }
     ...
     * @return A set of jobs that can now run since the specified job was
     * removed.
     */
    synchronized public Set<Job> removeJob(Job job) {
        if (!jobsWithAllRequiredLocks.contains(job)) {
            throw new RuntimeException("Trying to remove job without " +
                    "having all the locks");
        }
