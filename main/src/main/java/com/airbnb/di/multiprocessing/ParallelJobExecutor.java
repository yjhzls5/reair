...
/**
 * Accepts a bunch of jobs, executes them in parallel while observing the
 * locks that each jobs needs.
 */
public class ParallelJobExecutor {
     ...
     * running. This removes the job from the DAG so that other jobs that
     * depended on the finished job can now be run.
     * @param doneJob
     */
    ...
    /**
     * This is used with incrementJobDoneCount() to know when all the jobs
     * submitted to the executor has finished.
     */
    ...
    /**
     * Wait for the number of finished jobs to equal to the number of submitted
     * jobs.
     */
    ...
    synchronized public void start() {
        if (workers.size() > 0) {
            throw new RuntimeException("Start called while there are workers" +
                    " still running");
        }
