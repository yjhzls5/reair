...
/**
 * A Job is anything that needs to run, along with a set of pre-requisites.
 * In this case, the prerequisites are represented as a set of shared/exclusive
 * locks.
 */
public abstract class Job {

    /**
     * Before the Job runs, it needs to acquire a set of shared or exclusive
     * locks. Multiple jobs can have the same shared lock, but only one job can
     * have an exclusive one.
     */
...
//    abstract public Set<String> getRequiredSharedLocks();
    ...
    /**
     * Removes a parent job from this job's set of parent jobs. This should be
     * called when the parent job has finished running.
     * @param parentJob
     */
    public void removeParentJob(Job parentJob) {
        if (!parentJobs.contains(parentJob)) {
            throw new RuntimeException("Tried to remove job " + parentJob
                    + " when it wasn't a parent");
        }
    ...
    /**
     * Removes a child job from this job's set of child jobs. This should be
     * called when the this job has finished running and is being removed from
     * the DAG.
     * @param childJob
     */
    public void removeChildJob(Job childJob) {
        if (!childJobs.contains(childJob)) {
            throw new RuntimeException("Tried to remove job " + childJob
                    + " when it wasn't a child");
        }
