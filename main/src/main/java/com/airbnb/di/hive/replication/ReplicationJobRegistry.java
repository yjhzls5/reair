...
public class ReplicationJobRegistry {
    ...
    /**
     *
     * @return a collection containing all the active replication jobs. The
    ...
    synchronized public boolean retireJob(ReplicationJob job) {
        ...
        if (removedJob == null) {
            throw new RuntimeException("Couldn't find id: " + job.getId() +
                    " in the registry!");
        }

        if (removedJob != job) {
            throw new RuntimeException("Replication jobs with the same ID " +
                    "are not equal: %s and %s");
        }
