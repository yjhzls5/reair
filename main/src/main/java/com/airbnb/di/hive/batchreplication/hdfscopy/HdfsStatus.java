...
public class HdfsStatus extends Configured implements Tool {
    ...
    public static class ListFileMapper extends Mapper<Text, Boolean, Text, Text> {
        ...
        private long[] enumDirectories(FileSystem fs, Path root, boolean recursive, Context context) throws IOException {
            try {
                // keep track of file couter and total size
                long[] folderStats = new long[2];
                for (FileStatus status : nofilter?fs.listStatus(root):fs.listStatus(root, hiddenFileFilter)) {
                    if (status.isDir()) {
                        if (recursive) {
                ...
                try {
                    ...
                    context.write(new Text(parentPath),
                            new Text(genValue(relPath, String.valueOf(folderStats[0]), String.valueOf(folderStats[1]),
        ...
        protected void setup(Context context) throws IOException, InterruptedException {
            ...
            this.nofilter = context.getConfiguration().getBoolean(DirScanInputFormat.NO_FILE_FILTER, false);
        }
