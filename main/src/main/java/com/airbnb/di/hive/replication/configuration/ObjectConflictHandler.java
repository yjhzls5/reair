...
/**
 * Handles cases when there is an existing table or partition on the destination
 * cluster. This class can later be configured to be user-defined.
 */
public class ObjectConflictHandler implements Configurable {
    ...
    public boolean handleCopyConflict(Cluster srcCluster,
        ...
        if (!srcTable.getPartitionKeys().equals(
            ...
            LOG.warn("Not dropping data at location " +
                    ReplicationUtils.getLocation(existingDestTable));
    ...
    private boolean locationOnS3(StorageDescriptor sd) {
        ...
        return location != null &&
                (location.startsWith("s3n") || location.startsWith("s3a"));
    }
}
