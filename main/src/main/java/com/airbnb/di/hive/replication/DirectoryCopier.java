...
public class DirectoryCopier {
    ...
    public DirectoryCopier(Configuration conf,
                           Path tmpDir,
                           boolean checkFileModificationTimes) {
        this.conf = conf;
        this.tmpDir = tmpDir;
     ...
     * @param srcDir
     * @param destDir
     * @param copyAttributes a list of attributes to use when creating the tmp
     *                       directory. Doesn't really matter, but it can make
     *                       it easier to manually inspect the tmp directory.
     * @return
     * @throws IOException
     */
    public long copy(Path srcDir, Path destDir, List<String> copyAttributes)
        ...
        PathBuilder tmpDirPathBuilder = new PathBuilder(tmpDir)
                .add("distcp_tmp");
        for (String attribute : copyAttributes) {
            tmpDirPathBuilder.add(attribute);
        }
        Path distCpTmpDir = tmpDirPathBuilder
                .add(Long.toHexString(randomLong))
                .toPath();

        PathBuilder logDirPathBuilder = new PathBuilder(tmpDir)
                .add("distcp_logs");
        ...
        try {
            ...
            DistCpWrapperOptions options = new DistCpWrapperOptions(
                    srcDir,
                    destDir,
                    distCpTmpDir,
                    distCpLogDir)
