...
/**
 * Specification for a Hive object (table or partition). Used because having
 * 3 arguments (db, table, partition) for every function gets old.
 */
public class HiveObjectSpec {
    ...
    public HiveObjectSpec(NamedPartition p) {
        this(p.getPartition().getDbName(), p.getPartition().getTableName(),
                p.getName());
    }
    ...
    public HiveObjectSpec getTableSpec() {
        if (!isPartition()) {
            throw new RuntimeException("Should only be called for " +
                    "partition specs!");
        }
