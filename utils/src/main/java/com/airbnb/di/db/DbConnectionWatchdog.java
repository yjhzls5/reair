...
/**
 * Periodically checks to see if it's possible to make a connection to the DB.
 * If not, it exits this process. This is to handle a case where for some
 * reason, the MySQL JDBC driver gets in a bad state and is no longer able to
 * make connections. The only solution seems to be a restart.
 */
public class DbConnectionWatchdog extends Thread {

    private static final Log LOG = LogFactory.getLog(
            DbConnectionWatchdog.class);
    ...
    public void run() {
        ...
        while (true) {
            ...
            long timeSinceLastSuccessfulConnection = System.currentTimeMillis()
                    - lastSuccessfulConnectionTime;
            if (timeSinceLastSuccessfulConnection > WATCHDOG_TIMER_LIMIT) {
                LOG.error(String.format("Too much time has elapsed since the " +
                        "last successful DB connection (Elapsed: %sms " +
                        "Limit: %sms). Exiting...",
                        timeSinceLastSuccessfulConnection,
                        WATCHDOG_TIMER_LIMIT));
                System.exit(-1);
            }
            try {
