...
/**
 * Utility to re-run a method in case it throws transient exception.
 * E.g. when inserting into a DB, the DB might temporarily be down or
...
public class RetryingTaskRunner {
     ...
     * @param baseSleepInterval wait {@code baseSleepInterval}*2^(attempt no.)
     *                          between attempts.
     * @param maxSleepInterval wait {@code }
     */
    public RetryingTaskRunner(int defaultNumAttempts, int baseSleepInterval,
    ...
    public RetryingTaskRunner() {
        this(DEFAULT_NUM_ATTEMPTS, DEFAULT_BASE_SLEEP_INTERVAL,
    ...
    public void runWithRetries(RetryableTask q) throws Exception {
        ...
        for(int i = 0; i < numAttempts; i++) {
            try {
                ...
                LOG.error("Got an exception! Sleeping for " + sleepTime +
                        " seconds and retrying.", e);
